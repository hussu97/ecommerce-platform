...
Use the existing [docker-compose.yml](docker-compose.yml) and [docs/DOCKER.md](docs/DOCKER.md). For **production**, use PostgreSQL instead of SQLite and inject secrets via env.
...
1. **PostgreSQL**  
   Add a `postgres` service to Compose or use a managed PostgreSQL (e.g. Render, Supabase). All services that need the DB use the same `DATABASE_URL`.

2. **Secrets and env**  
   - Copy `.env.docker.example` to `.env` (or use a production env file).  
   - Set `DATABASE_URL=postgresql+asyncpg://...` for all API and worker services.  
   - Set `SECRET_KEY`, `BACKEND_CORS_ORIGINS` (comma-separated list of frontend origins).  
   - If using Stripe, set the required keys in customer-api.
...
4. **Run**  
   ...
   ```
   Ensure init has run (check logs). Frontends proxy to APIs inside the network; expose only what you need (e.g. reverse proxy in front of 3002 and 5174, or APIs directly).

5. **Reverse proxy (recommended)**  
   Put Nginx, Caddy, or Traefik in front to terminate HTTPS and route to customer-web (3002), admin-web (5174), customer-api (8002), admin-api (8003).
...
Deploy frontends and APIs to separate platforms. Good for beta; some services sleep when idle.
...
Host the full stack on Google Cloud using managed services: Cloud Run (APIs and optionally frontends), Cloud SQL (PostgreSQL), Secret Manager. Good for production; billing must be enabled (free tier still applies to many products).
...
4. **Build and push container images**  
   Use **Cloud Build** (trigger from your repo or run manually) to build the Docker images for customer-api and admin-api using the existing [customer-api/Dockerfile](customer-api/Dockerfile) and [admin-api/Dockerfile](admin-api/Dockerfile). For customer-web and admin-web, use [customer-web/Dockerfile](customer-web/Dockerfile) and [admin-web/Dockerfile](admin-web/Dockerfile) (build context must be repo root for design-tokens). Push images to **Artifact Registry** (or Container Registry). Ensure `asyncpg` is in admin-api’s requirements when using PostgreSQL.
...
7. **Run database migrations / seed**  
   One-time: run `reset_and_seed.py` against the production DB. Options: (a) Run from your machine using **Cloud SQL Proxy** and the DB connection string; (b) Run as a **Cloud Run Job** (same customer-api image, command override to run the seed script) with `DATABASE_URL` from Secret Manager; (c) A Cloud Build step that runs the seed with the DB URL from Secret Manager.

8. **Deploy customer-web**  
   **Option A:** Build a Next.js standalone Docker image and deploy it to Cloud Run (Node server). **Option B:** If using static export (`next export`), upload the output to a **Cloud Storage** bucket and serve via **Load Balancer** or **Firebase Hosting**. Document which approach you use so CORS and env (e.g. `NEXT_PUBLIC_API_URL`) are set at build time.

9. **Deploy admin-web**  
   Run `npm run build` (from repo root with admin-web and design-tokens). Upload the built static files to a **Cloud Storage** bucket. Configure the bucket for static website hosting, or put a **Load Balancer** or **Firebase Hosting** in front of the bucket. Set `VITE_API_URL` to the admin-api Cloud Run URL when building.
...
# Customer Web: http://127.0.0.1:3002, Admin Web: http://127.0.0.1:5174
# APIs: http://127.0.0.1:8002, http://127.0.0.1:8003
```

See [ARCHITECTURE.md](ARCHITECTURE.md) for ports and [docs/DOCKER.md](docs/DOCKER.md) for Docker details.
