...
export default function EditAddressScreen() {
  const { addressCode } = useLocalSearchParams<{ addressCode: string }>();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await api.get<SavedAddress>(`/addresses/${addressCode}`);
        const a = r.data;
        setForm({
          contact_name: a.contact_name,
          phone: a.phone,
          address_type: a.address_type as (typeof ADDRESS_TYPES)[number],
          street: a.street,
          city: a.city,
          country: a.country,
  ...
  }, [isAuthenticated, addressCode, router]);
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      await api.put(`/addresses/${addressCode}`, {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { borderBottomColor: colors.sandDivider }]}>
        ...
        <Text style={[styles.headerTitle, { fontFamily: FontFamily.serif, color: colors.text }]}>{t("edit")}</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
        ...
        <TouchableOpacity
          ...
          disabled={saving}
        >
          <Text style={styles.submitBtnText}>{saving ? t("processing") : t("submit")}</Text>
        </TouchableOpacity>
      </ScrollView>
    </View>
  );
