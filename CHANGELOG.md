...
- **API error display (customer-web):** All API error handling now uses `getApiErrorDetail()` so validation/error objects are never rendered as React children; login, signup, and profile address forms show a single string message. Backend access: server-side uses `BACKEND_ORIGIN` (Docker: `http://customer-api:8000`) or `http://localhost:8000/v1`; browser uses rewrites/proxy so frontends work locally and in Docker. (customer-web)
